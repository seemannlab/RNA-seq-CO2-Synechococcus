...
# But run in combination with the M_30 StringApp results
...
mcl |>
  write_tsv('analysis/M2_mcl-clustering.tsv')
...
c(
  ...
  sprintf(
    'table export outputFile="%s/analysis/M2_cluster-enrichment/cluster_%g.csv" table="STRING Enrichment: __mclCluster %g"',
    getwd(), x, x
  )
) |>
  write_lines('analysis/M2_cluster-enrichment-export-commands.txt')

dir.create('analysis/M2_cluster-enrichment/')
tryCatch({
...
string.enrichment <-
  'analysis/M2_cluster-enrichment/*csv' |>
  Sys.glob() %>%
...
# save for later use
enrich |>
  write_tsv('analysis/M2_string-enrichment.tsv')

set2genes |>
  write_tsv('analysis/M2_string-enrichment-genes.tsv')
